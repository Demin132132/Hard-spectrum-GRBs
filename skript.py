...
def graf(G, T, BG, fig, canal, Range, T100):
	...
	while i<len(S)-1:
		...
		"""
		z = np.linspace(0, max(G_), 10)
		d = np.linspace(T100, T100, 100)
		fig.append_trace(
			go.Scatter(
			x=d,
			y=z,
			line=dict(color='red', width=4, dash='0.1px')
			),
			row=row, col=col
			)
		"""
...
def main():
	...
	for ID in ID_:
		...
		for canal in ['G2', 'G3', 'G4', 'G5', 'G6', 'G7']:
			G, f = get_canal (PHA1, PHA2, PHA3, data.loc[canal], CAL[ID], T, canal, ID)
